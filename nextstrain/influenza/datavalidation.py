...
Validations:
  ...
  2) Name/strain column: must NOT contain invalid characters (default: apostrophe ' and backslash \\),
     because augur/iqtree will fail on those names.

Reads:   .xls/.xlsx/.xlsm/.tsv/.csv
Writes:  .xls (Excel 97–2003) using xlwt directly (works with pandas 2.x)
...
Examples:
  # Your typical case (keep column names)
  python3 datavalidation.py \
    --input data/h1n1pdm/metadata.xls \
    --output metadata.c.xls \
    --date-cols Collection_Date \
    --strain-col Isolate_Name \
    --require-all

  # Validate both collection + submission dates, and also remove invalid names
  python3 datavalidation.py \
    --input data/h1n1pdm/metadata.xls \
    --output metadata.c.xls \
    --date-cols Collection_Date,Submission_Date \
    --strain-col Isolate_Name \
    --require-all
"""
...
def validate_strain_names(series: pd.Series, invalid_pattern: re.Pattern) -> pd.Series:
    """
...
def main() -> int:
    ...
    ap.add_argument("--output", required=True, help="Output file (must end with .xls)")
    ...
    if out_path.suffix.lower() != ".xls":
        print("[ERROR] Output must be .xls (Excel 97–2003).", file=sys.stderr)
        return 2

    df = read_table(in_path)
    df.columns = [c.strip() for c in df.columns]
    df = df.reset_index(drop=True)  # ensure 0..n-1 for reason indexing
    ...
    strain_drop_mask = pd.Series(False, index=df.index)
    strain_col = args.strain_col.strip()

    if not args.skip_strain_validation:
        if strain_col not in df.columns:
            print(f"[ERROR] Missing strain/name column: {strain_col}", file=sys.stderr)
            print(f"Available columns: {list(df.columns)}", file=sys.stderr)
            return 2
        invalid_pat = re.compile(args.invalid_strain_pattern)
        strain_drop_mask = validate_strain_names(df[strain_col], invalid_pat)

    # --- COMBINE + REASONS ---
    drop_mask = date_drop_mask | strain_drop_mask
    ...
    if not args.skip_strain_validation:
        for i in strain_drop_mask[strain_drop_mask].index:
            add_reason(reasons, int(i), f"invalid_name:{strain_col}")
    ...
    # Full report (all columns + reason)
    removed.to_csv(full_path, sep="\t", index=False)

    # Preview report
    preview_cols: List[str] = []
    for c in [strain_col, "Isolate_Id", "Isolate_Name"]:
    ...
    # Output cleaned .xls (same columns as input)
    write_xls(kept, out_path)
    ...
    if not args.skip_strain_validation:
        print(f"[INFO] Invalid names in '{strain_col}' (pattern {args.invalid_strain_pattern!r}): {int(strain_drop_mask.sum())}")
    print(f"[INFO] Removed rows preview: {preview_path}")
    print(f"[INFO] Removed rows full: {full_path}")
    print(f"[INFO] Wrote cleaned .xls to: {out_path}")
    return 0
