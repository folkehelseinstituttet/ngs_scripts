...
# Repair common UTF-8/latin1 mojibake from upstream DB
...
# fludb comes from sourced script
...
# Ensure merged date column is Date
merged_df$prove_tatt <- parse_date_col(merged_df$prove_tatt)

# Replace missing / blank GISAID_Nr values
...
tmp <- merged_df %>%
  add_column(
    ...
    "Location" = ifelse(
      grepl("Ref", merged_df$prove_kategori),
      NA,
      paste0("Europe / Norway / ", merged_df$pasient_fylke_name)
    ),
    ...
    "Collection_Month" = lubridate::month(merged_df$prove_tatt),
    "Collection_Year" = lubridate::year(merged_df$prove_tatt),
...
# Function to write FASTA files and use Isolate_Name in headers
...
copy_folder <- function(source_dir, target_dir) {
  if (dir.exists(source_dir)) {
    ...
    for (file in files) {
      rel_path <- gsub(
        paste0("^", normalizePath(source_dir, winslash = "/")),
        "",
