...
write_fasta <- function(output_path, filtered_data) {
  ...
  for (i in 1:nrow(filtered_data)) {
    ...
    } else if (filtered_data$experiment[i] == "PB2") {
      header <- gsub("^PB2\\|", "", header)
    } else if (filtered_data$experiment[i] == "PB1") {
      header <- gsub("^PB1\\|", "", header)
    } else if (filtered_data$experiment[i] == "PA") {
      header <- gsub("^PA\\|", "", header)
    } else if (filtered_data$experiment[i] == "M") {
      header <- gsub("^M\\|", "", header)
    } else if (filtered_data$experiment[i] == "NP") {
      header <- gsub("^NP\\|", "", header)
    } else if (filtered_data$experiment[i] == "NS") {
      header <- gsub("^NS\\|", "", header)
    }
...
filtered_h1_pb2 <- filtered_h1 %>% filter(experiment == "PB2")
filtered_h1_pb1 <- filtered_h1 %>% filter(experiment == "PB1")
filtered_h1_pa <- filtered_h1 %>% filter(experiment == "PA")
filtered_h1_ns <- filtered_h1 %>% filter(experiment == "NS")
filtered_h1_mp <- filtered_h1 %>% filter(experiment == "MP")
filtered_h1_np <- filtered_h1 %>% filter(experiment == "NP")
...
output_fasta_h1_pb2 <- file.path(output_dir_h1, "raw_sequences_pb2.fasta")
output_fasta_h1_pb1 <- file.path(output_dir_h1, "raw_sequences_pb1.fasta")
output_fasta_h1_pa <- file.path(output_dir_h1, "raw_sequences_pa.fasta")
output_fasta_h1_ns <- file.path(output_dir_h1, "raw_sequences_ns.fasta")
output_fasta_h1_mp <- file.path(output_dir_h1, "raw_sequences_mp.fasta")
output_fasta_h1_np <- file.path(output_dir_h1, "raw_sequences_np.fasta")
...
write_fasta(output_fasta_h1_pb2, filtered_h1_pb2)
write_fasta(output_fasta_h1_pb1, filtered_h1_pb1)
write_fasta(output_fasta_h1_pa, filtered_h1_pa)
write_fasta(output_fasta_h1_mp, filtered_h1_mp)
write_fasta(output_fasta_h1_np, filtered_h1_np)
write_fasta(output_fasta_h1_ns, filtered_h1_ns)
